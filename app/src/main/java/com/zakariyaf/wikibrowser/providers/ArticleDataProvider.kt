...
class ArticleDataProvider {
    ...
    fun search(term: String, skip: Int, take: Int, respondeHandler: (result: WikiResult) -> Unit?) {
        Urls.getSearchUrl(term, skip, take).httpGet()
            .responseObject(WikipediaDataDeserializer()) { _, response, result ->
                ...
                val (data, _) = result
                respondeHandler.invoke(data as WikiResult)
            }
    }

    fun getRandom(take: Int, respondeHandler: (result: WikiResult) -> Unit?) {
